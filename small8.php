...
if(is_dir($fn))
{
echo('gugu<a href=\'#\' onclick=\'document.list.work_dir.value="'.$e_work_dir.str_replace('"','&quot;',$fn).'";document.list.submit();\'><b>'.htmlspecialchars(strlen($fn)>format?substr($fn,0,format-3).'...':$fn).'</b></a>'.str_repeat(' ',format-strlen($fn)));
if($winda===false)
{
...
{
$fn=$not_dirs[$i];
echo('<a href=\'#\' onclick=\'document.list.work_dir.value="'.(is_link($work_dir.$fn)?$e_work_dir.readlink($work_dir.$fn):$e_work_dir.str_replace('"','&quot;',$fn)).'";document.list.submit();\'>'.(is_link($work_dir.$fn)?$e_work_dir.readlink($work_dir.$fn):$e_work_dir.str_replace('"','&quot;',$fn)).'</a>'.str_repeat(' ',format-strlen($fn))); 
if($winda===false)
{
//$owner=@posix_getpwuid(@fileowner($work_dir.$fn));
//$group=@posix_getgrgid(@filegroup($work_dir.$fn));
...
//echo(@get_perms($work_dir.$fn).str_repeat(' ',10));
//printf("% 20s ",@filesize($work_dir.$fn).'B');
printf("% -20s",@date('M d Y',@filemtime($work_dir.$fn))."\n");
}
echo('</pre><hr>');
?>
